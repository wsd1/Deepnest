...
(function (root) {
	...
	function SvgNest() {
		...
		this.launchWorkers = function (tree, binPolygon, config, progressCallback, displayCallback) {
			...
			var i, j;
			//第一次进来 初始化GA 并增序组织tree
			if (GA === null) {
				// initiate new GA
				var adam = tree.slice(0); //复制一个数组
			...
			//上面连一个 individual 的 .fitness 都不满足
			if (individual === null) {
		...
		this.getParts = function (paths) {
			...
			for (i = 0; i < numChildren; i++) {
				var poly = SvgParser.polygonify(paths[i]);	  //从各种svg的图形标记（polyline polygon cirecle rectangle ...）转换成[{x,y},...] 点序列格式
		...
		this.cleanPolygon = function (polygon) {
			...
			//交叉线条 simplify 可能会产生若干个线条序列，这里找最大的
	...
	function GeneticAlgorithm(adam, bin, config) {
		...
		// population是一个个体数组，每一个个体都是一个对象，代表部件的角度和插入次序
		var angles = [];
